...
class GameScene: SKScene,SKPhysicsContactDelegate {
    ...
    //工厂方法，快速创建一个关卡并立即设置它的序数 |Factory method, quickly create a level and immediately set its ordinal number
    class func scene(_ size:CGSize, levelNumber:Int) -> GameScene {
        print(4)
        return GameScene(size: size, levelNumber: levelNumber)
    }
    //重写init，并将控制权转交给下面的init（简易构造器）。
    ...
    init(size:CGSize, levelNumber:Int) {
        ...
        backgroundColor = SKColor.lightGray //SKColor并不是真正的类，它是一种替身，相当于iOS和OX X之间移植的桥梁
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if location.y < frame.height * 0.2 { //触碰屏幕下方移动船
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if contact.bodyA.categoryBitMask == contact.bodyB.categoryBitMask {
            // 两种物理对象属于同一种物理类别 | Both bodies are in the same category
            let nodeA = contact.bodyA.node!
            let nodeB = contact.bodyB.node!

            // 这些节点能干什么？ | What do we do with these nodes?
            ...
            if contact.bodyA.categoryBitMask
                ...
                // Body A正在攻击Body B | Body A is attacking Body B
            ...
            // 我们要如何处理攻击者和被攻击者？ | What do we do with the attacker and the attackee?
